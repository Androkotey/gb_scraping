...
def get_data(url):
    ...
    # Проверяем в том числе, чтобы внутри нужного контейнера не было упоминаний про рекламу, потом возвраемся наверх
    ...
    for item in items:
        ...
        item_info['datetime'] = my.get_datetime_yandex(time)
        item_info['timestamp'] = my.get_timestamp_yandex(time)
